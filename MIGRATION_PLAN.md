...
Rationale: this is a solo portfolio project. Three separate pyproject files with a workspace coordinator (uv workspaces or PEP 517 path dependencies) adds ~30 lines of config and requires understanding workspace resolution. The optional-deps approach achieves identical Lambda isolation with the contract enforced by the import-isolation test in Step 4, not by package structure.
...
This replaces the current undocumented manual copy. A `build_lambda.sh` (or equivalent PowerShell) script will be documented in the README:
...
Do not proceed to Step 1 until this revised plan is approved.
